...
const Provas = () => {
    ...
    const fetchProvas = async () => {
        try {
            const response = await axios.get('https://api.comunhaorara.com:8080/treino/buscar');
    ...
    const handleUpdate = async () => {
        try {
            ...
            const response = await axios.patch(`https://api.comunhaorara.com:8080/treino/${editProvaId}`, {
    ...
    const handleDelete = async (id) => {
        try {
            const token = localStorage.getItem('token');
            const config = { headers: { Authorization: `${token}` } };
            await axios.delete(`https://api.comunhaorara.com:8080/treino/${id}`, config);

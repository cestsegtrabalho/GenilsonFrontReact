...
const Dados = () => {
    ...
    const fetchPerimetrias = async () => {
        try {
            const responsePerimetrias = await axios.get(`http://15.228.166.75:8080/perimetria/${userData._id}`);
    ...
    const fetchDobrasCutaneas = async () => {
        try {
            const responseDobrasCutaneas = await axios.get(`http://15.228.166.75:8080/dobrascutaneas/${userData._id}`);

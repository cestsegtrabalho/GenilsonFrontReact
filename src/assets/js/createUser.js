...
const CreateUser = () => {
    ...
    useEffect(() => {
        const checkUsernameAvailability = async () => {
            if (username) {
                try {
                    const response = await axios.get(`https://api.comunhaorara.com:8081/user/check-username/${username}`);
    ...
    const creteUser = async (e) => {
        e.preventDefault();
        try {
            const response = await axios.post(`https://api.comunhaorara.com:8081/user/criar`, {name, phone, email, username, password})

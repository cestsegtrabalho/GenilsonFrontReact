...
const CreateUser = () => {
    ...
    useEffect(() => {
        const checkUsernameAvailability = async () => {
            if (username) {
                try {
                    const response = await axios.get(`http://192.168.247.103:8080/user/check-username/${username}`);
    ...
    const creteUser = async (e) => {
        e.preventDefault();
        try {
            const response = await axios.post(`http://192.168.247.103:8080/user/criar`, {name, phone, email, username, password})

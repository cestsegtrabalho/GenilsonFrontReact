...
const CreateUser = () => {
    ...
    useEffect(() => {
        const checkUsernameAvailability = async () => {
            if (username) {
                try {
                    const response = await axios.get(`https://api.fittreinoapp.com/user/check-username/${username}`);
    ...
    const creteUser = async (e) => {
        e.preventDefault();
        try {
            const response = await axios.post(`https://api.fittreinoapp.com/user/criar`, {name, phone, email, username, password})
